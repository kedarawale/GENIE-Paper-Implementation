...
########################################################################
NODE2VEC_DIM = 256  # Increased to 256 to match yeast example
GCN_HIDDEN_DIM = 256
WATERMARK_RATE = 0.08
...
########################################################################
# 1. Data Loading (USAir97.mtx and USAir97.names)
########################################################################
def load_usair_data(mtx_file='USAir97.mtx'):
    """
    Loads the USAir dataset from the provided MTX file.
    Returns a CSR adjacency matrix (undirected and weighted).
    """
...
########################################################################
def build_or_load_embeddings(adj_matrix, embedding_file='usair_embeddings.npy', overwrite=False): # Updated embedding file name
    node2vec_model = None  # Initialize node2vec_model here
    if os.path.exists(embedding_file) and not overwrite:
        ...
        node2vec_model = node2vec.Node2Vec( # Assign to node2vec_model
            G_nx,
        ...
        model_n2v = node2vec_model.fit(window=10, min_count=1, batch_words=4)
...
def build_or_load_splits(data, split_file='usair_splits.pkl', overwrite=False): # Updated split file name
...
########################################################################
# 4. GCN Encoder, Decoder and Model (Unchanged)
########################################################################
...
########################################################################
# 5. Watermark Injection by Modifying Node Features and Edges (Unchanged)
########################################################################
...
########################################################################
# 6. Training & Evaluation Functions (for GCN) (Unchanged)
########################################################################
...
########################################################################
# 7. Dynamic Thresholding + Smoothed Bootstrap (same as SEAL code) (Unchanged)
########################################################################
...
########################################################################
# 8. Minimal Judge Class (Registry + Dispute) (same as SEAL code) (Unchanged)
########################################################################
class Judge:
    def __init__(self):
        self.registry = {}
...
########################################################################
# 9. Fine-Tune Attack (same as SEAL code, but using GCN eval) (Unchanged)
########################################################################
...
########################################################################
# 10. Main (updated for USAir)
########################################################################
if __name__ == "__main__":
    torch.cuda.empty_cache()

    # 1) Load adjacency
    adj_matrix = load_usair_data('USAir97.mtx') # Updated data loading function and file name
    ...
    embeddings = build_or_load_embeddings(adj_matrix, 'usair_embeddings.npy', overwrite=False) # Updated embedding file name
    ...
    train_data, val_data, test_data = build_or_load_splits(data_full, 'usair_splits.pkl', overwrite=False) # Updated splits file name
    ...
    # Setting up a judge
    judge = Judge()
    judge.register_model("USAir_GCNModelOwner", {"secret_info": "my_watermark"}) # Updated owner name
    ...
    judge.dispute_ownership("USAir_GCNModelOwner", attacked_model, wm_data, # Updated owner name
