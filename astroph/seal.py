...
########################################################################
def load_astroph_data(file_path='CA-AstroPh.txt'):
    """
    Loads an undirected adjacency matrix from 'CA-AstroPh.txt',
    with lines like:  84424	276
                     84424	1662
                     ...
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            if not line or line.startswith('#'):
                continue
            s_str, t_str = line.split('\t') # Split by tab
...
########################################################################
def build_or_load_embeddings(adj_matrix, embedding_file='astroph_embeddings.npy', overwrite=False): # changed embedding_file name
...
def build_or_load_splits(data, split_file='astroph_splits.pkl', overwrite=False): # changed split_file name
...
if __name__ == "__main__":
    ...
    # 1) Load adjacency
    adj_matrix = load_astroph_data('CA-AstroPh.txt') # changed data loading function and file name
    ...
    embeddings = build_or_load_embeddings(adj_matrix, 'astroph_embeddings.npy', overwrite=False) # changed embedding file name
    ...
    train_data, val_data, test_data = build_or_load_splits(data_full, 'astroph_splits.pkl', overwrite=False) # changed split file name
    ...
    # Setting up a judge
    judge = Judge()
    judge.register_model("AstroPhModelOwner", {"secret_info": "my_watermark"}) # Changed owner name
    ...
    judge.dispute_ownership("AstroPhModelOwner", attacked_model, loader_wm_only, # Changed owner name
