...
NODE2VEC_DIM = 256
GCN_HIDDEN_DIM = 256
WATERMARK_RATE = 0.125  # Increased watermark rate (0.1)
NEG_SAMPLING_RATIO = 0.5
EPOCHS = 400

WM_LOSS_WEIGHT = 2 # Reverted WM_LOSS_WEIGHT back to 2.0
...
########################################################################
# 1. Data Loading (inf-power.mtx)
########################################################################
def load_power_data(file_path='inf-power.mtx'):
    """
    Loads an undirected adjacency matrix from 'inf-power.mtx' (MatrixMarket format).
    """
    try:
        # Load the Matrix Market file
        mm = scipy.io.mmread(file_path)
        adj_matrix = mm.tocsr()

        # Ensure symmetry if it's supposed to be symmetric
        adj_matrix = adj_matrix + adj_matrix.T  # Ensure symmetry by adding transpose
        adj_matrix.data[:] = 1  # Make it binary adjacency (if not already)
        adj_matrix = adj_matrix.tocsr()

        return adj_matrix

    except Exception as e:
        raise Exception(f"Error loading inf-power dataset: {e}")
...
def build_or_load_splits(data, split_file='inf_power_splits.pkl', overwrite=False):
...
########################################################################
def train_step(model, optimizer, train_data, wm_data, device, wm_loss_weight=WM_LOSS_WEIGHT): # Added wm_loss_weight
    model.train()
    optimizer.zero_grad()
    ...
    loss_train_weighted = loss_train # No weighting for clean loss
    ...
    loss_wm_weighted = loss_wm * wm_loss_weight  # Apply weight to watermark loss

    # Combine losses and backpropagate
    total_loss = loss_train_weighted + loss_wm_weighted
    total_loss.backward()
    optimizer.step()
...
if __name__ == "__main__":
    ...
    # 1) Load adjacency
    adj_matrix = load_power_data('inf-power.mtx')
    ...
    embeddings = build_or_load_embeddings(adj_matrix, 'inf_power_embeddings.npy', overwrite=False)
    ...
    train_data, val_data, test_data = build_or_load_splits(data_full, 'inf_power_splits.pkl', overwrite=False)
    ...
    for ep in range(1, EPOCHS + 1):
        norm_loss, wm_loss = train_step(model, optimizer, train_data, wm_data, device, wm_loss_weight=WM_LOSS_WEIGHT) # Pass wm_loss_weight
        if ep % 50 == 0:
    ...
    # Setting up a judge
    judge = Judge()
    judge.register_model("Power_GCNModelOwner", {"secret_info": "my_watermark"})
    ...
    judge.dispute_ownership("Power_GCNModelOwner", attacked_model, wm_data,
