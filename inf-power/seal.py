...
WM_LOSS_WEIGHT = 5.0  # Emphasize watermark edges
...
########################################################################
def load_inf_power_data(file_path='inf-power.mtx'):
    """
    Loads an undirected adjacency matrix from 'inf-power.mtx',
    with lines like:  387 1
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            if not line or line.startswith('%'): # Skip comment lines
                continue
            if len(line.split()) == 3 and line.split()[0].isdigit() and line.split()[1].isdigit() and line.split()[2].isdigit(): # Skip header line with node/edge counts
                try:
                    num_nodes, _, num_edges = map(int, line.split())
                except ValueError:
                    continue # If conversion fails, treat as comment/header
                continue # Skip the line after reading node/edge counts

            s_str, t_str = line.split()
            s, t = int(s_str) - 1, int(t_str) - 1 # Subtract 1 for 0-based indexing
            src_list.append(s)
            tgt_list.append(t)
            if s != t:
                # Undirected
...
def bfs_khop_subgraph(u, v, adj_csr, k=1, max_nodes=50000):
    ...
    while queue:
        ...
        # Error fix (previous): Ensure cur + 1 is within the valid index range of adj_csr.indptr
        if cur + 1 >= adj_csr.indptr.shape[0]:
    ...
    for old_n in sub_nodes:
        ...
        # Error fix (current): Ensure old_n + 1 is within valid index range of adj_csr.indptr
        if old_n + 1 >= adj_csr.indptr.shape[0]:
...
if __name__ == "__main__":
    ...
    # 1) Load adjacency
    adj_matrix = load_inf_power_data('inf-power.mtx') # Changed to inf-power dataset
    ...
    embeddings = build_or_load_embeddings(adj_matrix, 'inf_power_embeddings.npy', overwrite=False) # Changed embedding file name
    ...
    train_data, val_data, test_data = build_or_load_splits(data_full, 'inf_power_splits.pkl', overwrite=False) # Changed splits file name
    ...
    # Setting up a judge
    judge = Judge()
    judge.register_model("InfPowerModelOwner", {"secret_info": "my_watermark"})
    ...
    judge.dispute_ownership("InfPowerModelOwner", attacked_model, loader_wm_only,
