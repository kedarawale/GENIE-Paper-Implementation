...
# 1. Data Loading (BIOGRID)

########################################################################
def load_biogrid_data(file_path='BIOGRID.txt'):
    """
    Loads an undirected adjacency matrix from 'BIOGRID.txt',
    using 'Entrez Gene Interactor A' and 'Entrez Gene Interactor B' columns.
    Returns a CSR adjacency matrix (undirected).
    """
    src_list, tgt_list = [], []
    node_to_id_map = {}  # Map gene IDs to sequential node IDs
    next_node_id = 0
    ...
    with open(file_path, 'r') as f:
        ...
        for line in f:
            ...
            if not line or line.startswith('#'):
                continue
            parts = line.split('\t')
            if len(parts) < 3: # Check if line has enough columns
                continue

            s_str, t_str = parts[1], parts[2] # Entrez Gene Interactor A and B
            try:
                s, t = int(s_str), int(t_str)
            except ValueError: # Handle cases where gene IDs are not integers
                continue # Skip lines with non-integer IDs

            if s not in node_to_id_map:
                node_to_id_map[s] = next_node_id
                next_node_id += 1
            if t not in node_to_id_map:
                node_to_id_map[t] = next_node_id
                next_node_id += 1

            s_id = node_to_id_map[s]
            t_id = node_to_id_map[t]

            src_list.append(s_id)
            tgt_list.append(t_id)
            if s_id != t_id: # Avoid self-loops if gene IDs are the same
                src_list.append(t_id) # Undirected
                tgt_list.append(s_id)
...
########################################################################
def build_or_load_embeddings(adj_matrix, embedding_file='biogrid_embeddings.npy', overwrite=False):
    if os.path.exists(embedding_file) and not overwrite:
        ...
        node2vec_model = node2vec.Node2Vec( # Initialize node2vec here
            G_nx,
...
def build_or_load_splits(data, split_file='biogrid_splits.pkl', overwrite=False):
...
# 10. Main (updated for GCN and BIOGRID)
...
if __name__ == "__main__":
    ...
    # 1) Load adjacency
    adj_matrix = load_biogrid_data('BIOGRID.txt')  # Corrected file name to BIOGRID.txt
    ...
    embeddings = build_or_load_embeddings(adj_matrix, 'biogrid_embeddings.npy', overwrite=True) # Force overwrite for this run
    ...
    # Setting up a judge
    judge = Judge()
    judge.register_model("BIOGRID_GCNModelOwner", {"secret_info": "my_watermark"}) # Changed owner name
    ...
    judge.dispute_ownership("BIOGRID_GCNModelOwner", attacked_model, wm_data, # Changed owner name
