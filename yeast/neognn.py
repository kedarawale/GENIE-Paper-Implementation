...
# Clear leftover GPU memory from any previous runs in the same process
...
# ---------------------------------------------------------------
NODE2VEC_DIM = 32        # Reduced dimension to save memory
GCN_HIDDEN_DIM = 32      # Also reduced hidden dimension from 64 to 32
WATERMARK_RATE = 0.01    # Watermark rate
...
def load_yeast_data(file_path='yeast.edges'):
    """
    Loads the yeast dataset and returns the adjacency matrix in csr format.
    """
    src_list, tgt_list = [], []
    max_node_id = -1
...
def build_or_load_embeddings(adj_matrix, embedding_file='yeast_embeddings.npy', overwrite=False):
    """
    Builds embeddings via Node2Vec or loads them if they already exist.
    """
...
def build_or_load_splits(data, split_file_path='yeast_splits.pkl', overwrite=False):
    """
    Builds or loads the train/val/test splits for link prediction.
    """
...
class NeoGNN(torch.nn.Module):
    """
    The main NeoGNN model, adapted for link prediction with watermarking.
    """
    ...
    def get_neighbor_embeddings(self, z, nodes):
        ...
        avoids adjacency matrix -> dense conversion.
        """
        neighbor_embeddings = []
        for node in nodes:
    ...
    for i in wm_nodes:
        for j in wm_nodes:
            ...
            if (i, j) in edge_set:
                ...
                # If edge didn't exist, label is 1, add it
                wm_edges.append([i, j])
                wm_labels.append(1)
                edge_set.add((i, j))
                edge_set.add((j, i))
...
def estimate_kde(data):
    """
    Fits a Kernel Density Estimator (KDE) on the given data.
    """
...
def sample_from_kde(kde_model, n_samples=5000):
    """
    Draw samples from the given KDE model.
    """
...
if __name__ == "__main__":
    ...
    embeddings = build_or_load_embeddings(
        adj_matrix,
        embedding_file='yeast_embeddings.npy',
        overwrite=False
    )
    ...
    train_data, val_data, test_data = build_or_load_splits(
        data,
        split_file_path='yeast_splits.pkl',
        overwrite=False
    )
    ...
    # 9. Build an empirical distribution for 'clean models' (Models trained WITHOUT watermark)
    ...
    # Re-verify watermark in the attacked model
